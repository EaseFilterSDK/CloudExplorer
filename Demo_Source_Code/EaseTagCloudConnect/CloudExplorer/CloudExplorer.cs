        ...
        private void CreateStubFile()
        {
            try
            {
                ...
                foreach (ListViewItem lvItem in lvFiles.SelectedItems)
                {
                    ...
                    try
                    {
                        bool ret = EaseTag.CreateStubFile(stubFileName, fileSize, fileAttributes, (uint)tagData.Length, Marshal.UnsafeAddrOfPinnedArrayElement(tagData, 0), true, ref fileHandle);
                        if (!ret)
                        {
