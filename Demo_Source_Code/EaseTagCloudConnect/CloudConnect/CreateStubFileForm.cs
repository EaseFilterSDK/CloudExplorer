...
namespace CloudConnect
{
    public partial class CreateStubFileForm : Form
    {
        ...
        private void button_CreateStubFile_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                bool ret = EaseTag.CreateStubFile(fileName, fileSize, fileAttributes, (uint)tagData.Length, Marshal.UnsafeAddrOfPinnedArrayElement(tagData, 0), true, ref fileHandle);
                if (!ret)
                {
                    MessageBox.Show("Create stub file:" + fileName + " failed.\n" + EaseTag.GetLastErrorMessage(), "CreateStubFile", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
            }
            finally
            {
                ...
                {
                    EaseTag.CloseHandle(fileHandle);
                }
            }
