...
namespace CloudConnect
{
    public class FilterService
    {
        ...
        /// <returns></returns>
        static bool DownloadCacheFile(EaseTag.MessageSendData messageSend, string cacheFileName,ref EaseTag.MessageReplyData messageReply)
        {
            bool ret = false;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (messageReply.DataBuffer != null && messageReply.DataBuffer.Length >= returnCacheFileName.Length * 2)
                    {
                        ...
                        {
                            messageReply.FilterStatus = (uint)EaseTag.FilterStatus.REHYDRATE_FILE_VIA_CACHE_FILE;
                        }
                        else
                        {
                            messageReply.FilterStatus = (uint)EaseTag.FilterStatus.CACHE_FILE_WAS_RETURNED;
                        }
                        
                        messageReply.ReturnStatus = (uint)EaseTag.NTSTATUS.STATUS_SUCCESS;
        ...
        /// <returns></returns>
        static bool GetRequestedBlockData(string cacheFileName, long offset, uint readLength, ref EaseTag.MessageReplyData messageReply)
        {
            bool ret = false;

            try
            {
                messageReply.ReturnStatus = (uint)EaseTag.NTSTATUS.STATUS_UNSUCCESSFUL;
                ...
                messageReply.ReturnStatus = (uint)EaseTag.NTSTATUS.STATUS_SUCCESS;
                messageReply.FilterStatus = (uint)EaseTag.FilterStatus.BLOCK_DATA_WAS_RETURNED;
        ...
        private static bool DownloadFileFromCloudStorage(EaseTag.MessageSendData messageSend,string testSourceFileName, ref EaseTag.MessageReplyData messageReply)
        {
            Boolean ret = false;
            try
            {
        ...
        public static Boolean ProcessRequest(EaseTag.MessageSendData messageSend, ref EaseTag.MessageReplyData messageReply)
        {
            Boolean ret = false;
            ...
            try
            {
                ...
                else if (messageSend.MessageType == (uint)EaseTag.MessageType.MESSAGE_TYPE_RESTORE_FILE_TO_CACHE)
                {
                ...
                else if (messageSend.MessageType == (uint)EaseTag.MessageType.MESSAGE_TYPE_RESTORE_BLOCK_OR_FILE)
                {
                ...
                if (messageReply.ReturnStatus != (uint)EaseTag.NTSTATUS.STATUS_SUCCESS)
                {
                ...
                EventManager.WriteMessage(169, "ProcessRequest", eventLevel, "Return MessageId#" + messageSend.MessageId
                         + " ReturnStatus:" + ((EaseTag.NTSTATUS)(messageReply.ReturnStatus)).ToString() + ",FilterStatus:" + messageReply.FilterStatus
