        ...
        static public bool CreateTestStubFiles(string folder)
        {
            ...
            try
            {
                ...
                foreach (string file in files)
                {
                    ...
                    try
                    {
                        ret = EaseTag.CreateStubFile(stubFileName, fileInfo.Length, (uint)FileAttributes.Offline, (uint)tagData.Length, Marshal.UnsafeAddrOfPinnedArrayElement(tagData, 0), true, ref fileHandle);
                        if (!ret)
                        {
                            Console.WriteLine("Create stub file:" + stubFileName + " failed.\n" + EaseTag.GetLastErrorMessage());
                            return ret;
                        }
                    }
                    finally
